...
def run_load_test(
    ...
    print(
        f"Starting load test: {workers} workers x {iters} requests = {workers * iters} total"
    )
...
def main() -> None:
    parser = argparse.ArgumentParser(
        description="Concurrent load test for the hybrid RAG API."
    )
    parser.add_argument(
        "--api-url", default="http://localhost:8000", help="Base URL of the running API"
    )
    parser.add_argument(
        "--workers", type=int, default=4, help="Number of concurrent worker threads"
    )
